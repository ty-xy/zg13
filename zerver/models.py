...
def get_system_bot(email: Text) -> UserProfile:
    ...
    # return UserProfile.objects.select_related().get(email__iexact=email.strip())
    try:
        user_obj = UserProfile.objects.get(id=1)
    except Exception:
        realm = Realm.objects.filter(id=1)
        user_obj = create_user(email='bot@zg18.com', password='123321',
                               realm=realm[0], full_name='机器人', short_name='机器人',
                               is_realm_admin=False, bot_type=4, bot_owner=None,
                               tos_version='Asia/Shanghai'
                               )
    return user_obj
...
# 报表评论：
class ZgStatementComment(models.Model):
    # 主题ID
...
# 评论回复
class ZgReplyComment(models.Model):
    comment_id = models.ForeignKey(ZgStatementComment)
    # 回复目标用户id
