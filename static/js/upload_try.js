var upload_try= (function () {
    ...
    exports.feature_check = function (upload_button) {
        if (window.XMLHttpRequest && (new XMLHttpRequest()).upload_try) {
    ...
    exports.options = function (config) { 
        ...
        var uploadFinished = function (i, file, response) {
            ...
            if (/MSIE/.test(navigator.userAgent)) {
                $('#' + file_input).replaceWith($('#' + file_inputs).clone(true));
            } else {
                $('#' + file_inputs).val('');
            }
        };
    ...
    if (window.bridge) {
        var opts = exports.options({ mode: "choose" });
